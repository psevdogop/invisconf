...
(function() {
  ...
  var EVENTS_NAMESPACE_ROUTER = 'uiRouting';
  ...
  function uiRouterStylesDirective($rootScope, $compile, $state, $interpolate, $document) {
    ...
    function uiRouterStylesLink(scope) {
      ...
      function initState(name) {
        switch(name) {
            case 'home':
                $rootScope.$broadcast(EVENTS_NAMESPACE_ROUTER + ':' + name);
                $rootScope.$broadcast(EVENTS_NAMESPACE_ROUTER + ':' + 'header');
                $rootScope.$broadcast(EVENTS_NAMESPACE_ROUTER + ':' + 'ability');
        }
      }
      ...
      function stateChangeSuccessCallback(evt, toState) {
        ...
        var stateRouter = $$parentState(toState);
        // console.log(stateRouter, toState);
        if (stateRouter) {
            initState(stateRouter.name);
        }
